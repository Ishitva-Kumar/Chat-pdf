  ...
  return (
    <div className="px-6 py-8 h-[100svh]">
      ...
      <div className="flex flex-col space-y-2">
        ...
        {files.map((file: any) => (
          <div key={file.fileUrl} className="border p-2 rounded flex justify-between items-center">
            <a href={file.fileUrl} target="_blank" rel="noopener noreferrer">
              {file.fileName}
            </a>
            <Link href={`/dashboard/chat?fileUrl=${encodeURIComponent(file.fileUrl)}&fileName=${encodeURIComponent(file.fileName)}`}>
              <button className="ml-4 px-2 py-1 bg-blue-500 text-white rounded">Chat</button>
            </Link>
          </div>
        ))}
      </div>
